...
- Any operational key material or external tool referenced by source must still undergo human provenance review.
...
3. Retain `go.sum` and commit Terraform provider lockfiles.
...
The checker detects tracked generated files, tracked binaries, empty tracked files, Terraform state files, and missing root provenance documents. It does not execute attack modules and does not modify the repository.
