...
func (e *Engine) persistState() {
	...
	state := storage.PersistentState{Version: 1, Jobs: e.jobController.List(), Evidence: evidence, Findings: findings, Reports: reports, ControllerPrivate: hex.EncodeToString(private), ControllerPublic: hex.EncodeToString(public)}
