...
func NewPortScanner(target string, timeout time.Duration, protocol string, threads int) *PortScanner {
return &PortScanner{
Target:   target,
Timeout:  timeout,
Protocol: protocol,
...
return p.ScanPortsWithDelay(ctx, commonPorts)
}

func (p *PortScanner) ScanPortsWithDelay(ctx context.Context, ports []int) ([]PortScanResult, error) {
var results []PortScanResult
for _, port := range ports {
res := p.ScanPort(ctx, port)
if res.Open {
results = append(results, res)
}
time.Sleep(p.Delay)
...
func (p *PortScanner) GetThreads() int {
return p.Threads
}
